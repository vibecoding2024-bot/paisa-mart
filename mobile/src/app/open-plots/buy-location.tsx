...
export default function BuyLocationScreen() {
      ...
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        <View className="px-4 pt-3">
          <View className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <View className="h-full w-[50%] bg-orange-500 rounded-full" />
          </View>
        </View>
        ...
        <View className="flex-1 px-6 pt-12">
          ...
          >
            <View className="w-20 h-20 bg-blue-100 rounded-full items-center justify-center">
              <MapPin size={40} color="#3B82F6" strokeWidth={2} />
            </View>
          </Animated.View>
          ...
          <Animated.View entering={FadeInDown.delay(300)}>
            <TextInput
              className="bg-white rounded-2xl px-6 py-5 text-gray-900 text-lg border-2 border-gray-200 focus:border-blue-500"
        ...
        <View className="px-6 pb-8">
          <Pressable
            ...
            className={`rounded-2xl py-5 items-center ${
              location.trim() ? 'bg-blue-500' : 'bg-gray-300'
            }`}
            style={{
              shadowColor: location.trim() ? '#3B82F6' : '#000',
