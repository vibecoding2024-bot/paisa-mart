...
import { ChevronLeft, ChevronDown, ChevronUp, Home, CheckCircle2 } from 'lucide-react-native';
      ...
      {open && (
        <Animated.View
          ...
          style={{
            ...
            shadowOffset: { width: 0, height: 4 }, elevation: 4, zIndex: 100,
          }}
        >
          {options.map((opt, idx) => (
            <Pressable
              key={idx}
              onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); onSelect(opt); setOpen(false); }}
              style={{
            ...
            >
              <Text style={{ fontSize: 14, color: value === opt ? '#002561' : '#374151', fontWeight: value === opt ? '600' : '400' }}>
                {opt}
              </Text>
